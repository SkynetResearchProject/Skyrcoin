...
# Compile dependencies
	cd depends
	make -j$(echo $CPU_CORES) HOST=aarch64-linux-gnu
	cd ..

# Compile
	./autogen.sh
	#./configure --prefix=$PWD/depends/aarch64-linux-gnu --enable-glibc-back-compat --enable-reduce-exports LDFLAGS=-static-libstdc++ --disable-shared --disable-debug --disable-tests #--disable-wallet
        ...
        #set make -j1 if 'if you get an error: "aarch64-linux-gnu-g++: fatal error: Killed signal terminated program cc1pluscompilation terminated'
        make -j$(echo $CPU_CORES) HOST=aarch64-linux-gnu
	cd ..

# Create zip file of binaries
	cp Skyrcoin/src/skyrcoind Skyrcoin/src/skyrcoin-cli Skyrcoin/src/skyrcoin-tx Skyrcoin/src/qt/skyrcoin-qt .
	zip SKYR-$(git describe --abbrev=0 --tags | sed s/v//)-AARCH64-Linux.zip skyrcoind skyrcoin-cli skyrcoin-tx skyrcoin-qt
