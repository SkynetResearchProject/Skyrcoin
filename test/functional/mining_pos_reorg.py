...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected SKYR and zSKYR supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zSKYRsupply']:
                assert_equal(node['zSKYRsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check SKYR and zSKYR supply at the beginning
        # ------------------------------------------
        # zSKYR supply: 2 coins for each denomination
        ...
        # SKYR supply: block rewards minus burned fees for minting
        ...
        # Verify that SKYR and zSKYR supplies were properly updated after the spends and reorgs
        self.log.info("Check SKYR and zSKYR supply...")
