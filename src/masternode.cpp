...
CAmount CMasternode::GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 1) {
        ...
        nSubsidy = 0.5 * COIN;    
    } else if (nHeight > 800000 && nHeight < 1000000) {
        nSubsidy = 0.5 * COIN;
        ...
        nSubsidy = 2.5 * COIN;
    } else if (nHeight >= 6000000 && nHeight < 8000000) {
        nSubsidy = 1.25 * COIN;
    } else if (nHeight >= 8000000 && nHeight < 10000000) {
        nSubsidy = 0.63 * COIN;
    } else if (nHeight >= 10000000 && nHeight < 12000000) {
        nSubsidy = 0.32 * COIN;
    } else if (nHeight >= 12000000 && nHeight < 14000000) {
        nSubsidy = 0.16 * COIN;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 SKYR tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = UINT256_ZERO;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 SKYR tx -> 1 confirmation
