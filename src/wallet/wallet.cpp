...
/**
 * Fees smaller than this (in uSKYR) are considered zero fee (for transaction creation)
        ...
        if (zwallet) {
            ...
            if (CWalletDB(strWalletFile).ReadCurrentSeedHash(hashSeed)) {
                ...
                if (!GetDeterministicSeed(hashSeed, nSeed)) {
                    return error("Failed to read zSKYR seed from DB. Wallet is probably corrupt.");
                }
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate, bool fromStartup)
{
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            // Will try to rescan it if zSKYR upgrade is active.
...
CAmount CWallet::GetBalance(bool fIncludeDelegated) const // <- new
{
...
CAmount CWallet::GetAvailableBalance(isminefilter& filter, bool useCache, int minDepth) const
{
    return loopTxsBalance([filter, useCache, minDepth](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal){
        ...
        if (pcoin.IsTrusted(depth, fConflicted) && depth >= minDepth) {
            ...
            //if (!fIncludeDelegated) nTotal -= pcoin.GetStakeDelegationCredit(); //from pivx
        }
    });
}
...
CAmount CWallet::GetStakingBalance(const bool fIncludeColdStaking) const
{
    return std::max(CAmount(0), loopTxsBalance(
        ...
        if (pcoin.IsTrusted() && pcoin.GetDepthInMainChain() 
		>= 
	   (Params().GetConsensus().NetworkUpgradeActive(chainActive.Height(), Consensus::UPGRADE_STAKE_MIN_DEPTH_V2) ?
                    Params().GetConsensus().nStakeMinDepthV2 : Params().GetConsensus().nStakeMinDepth)
        ) {
...
CAmount CWallet::GetDelegatedBalance() const //<--new
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            if (pcoin.HasP2CSOutputs() && pcoin.IsTrusted())
...
{
    const bool fIncludeCold = (sporkManager.IsSporkActive(SPORK_17_NOOP) &&
                               GetBoolArg("-coldstaking", true));
    return AvailableCoins(pCoins,
            nullptr,            // coin control
            false,              // IncludeDelegated
            fIncludeCold,       // IncludeColdStaking
...

    {
            ...
            while (true) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-SKYR-address
        ...
        )
{
    ...
    LogPrintf("STAKING %s: attempted staking %d times\n", __func__, nAttempts);
...
std::string CWallet::GetWalletHelpString(bool showDebug)
{
    ...
    strUsage += HelpMessageOpt("-minstakesplit=<amt>", strprintf(_("Minimum positive amount (in SKYR) allowed by GUI and RPC for the stake split threshold (default: %s)"), FormatMoney(DEFAULT_MIN_STAKE_SPLIT_THRESHOLD)));
...
CWallet* CWallet::CreateWalletFromFile(const std::string walletFile)
{
    ...
    if (nLoadWalletRet != DB_LOAD_OK) {
        if (nLoadWalletRet == DB_CORRUPT) {
            ...
            UIError(strprintf(_("Error loading %s: Wallet requires newer version of Skyrcoin"), walletFile));
            return nullptr;
        } else if (nLoadWalletRet == DB_NEED_REWRITE) {
            UIError(_("Wallet needed to be rewritten: restart Skyrcoin to complete"));
            return nullptr;
        } else {
    ...
    if (!zwalletInstance->GetMasterSeed().IsNull()) {
        //Inititalize zSKYRWallet
        uiInterface.InitMessage(_("Syncing zSKYR wallet..."));
