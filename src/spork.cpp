...
std::vector<CSporkDef> sporkDefs = {
    ...
    MAKE_SPORK_DEF(SPORK_17_NOOP,                           4070908800ULL), // OFF //SPORK_17_COLDSTAKING_ENFORCEMENT
    ...
    MAKE_SPORK_DEF(SPORK_107_ALLOW_DUPLICATE_MN_IPS,        4070908800ULL), // OFF
};
...
// Skyrcoin: on startup load spork values from previous session if they exist in the sporkDB
...
void CSporkManager::ProcessSpork(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == NetMsgType::SPORK) {
        ...
        // Skyrcoin: add to spork database.
