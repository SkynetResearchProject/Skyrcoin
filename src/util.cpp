...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\skyrcoin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\skyrcoin
// Mac: ~/Library/Application Support/skyrcoin
// Unix: ~/.skyrcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "skyrcoin";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\skyrcoinParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\skyrcoinParams
    // Mac: ~/Library/Application Support/skyrcoinParams
    // Unix: ~/.skyrcoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "skyrcoinParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "skyrcoinParams";
#else
    // Unix
    return pathRet / ".skyrcoin-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty skyrcoin.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override skyrcoin.conf
