...
/** Fees smaller than this (in uSKYR) are considered zero fee (for relaying, mining and transaction creation)
                              ...
                              std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            // Reject legacy zSKYR mints
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zSKYR mint output in tx %s",
...
DisconnectResult DisconnectBlock(CBlock& block, CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    //Track zSKYR money supply
    if (!UpdateZPIVSupplyDisconnect(block, pindex)) {
        error("%s: Failed to calculate new zSKYR supply", __func__);
        return DISCONNECT_FAILED;
    }
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    // Update zSKYR money supply map
    if (!UpdateZPIVSupplyConnect(block, pindex, fJustCheck)) {
        return state.DoS(100, error("%s: Failed to calculate new zSKYR supply for block=%s height=%d", __func__,
                                    block.GetHash().GetHex(), pindex->nHeight), REJECT_INVALID);
    }

    // A one-time event where the zSKYR supply was off (due to serial duplication off-chain on main net)
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zSKYR spends in this block
        if (tx.HasZerocoinSpendInputs()) {
            for (const CTxIn& txIn : tx.vin) {
                bool isPublicSpend = txIn.IsZerocoinPublicSpend();
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zSKYR serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        return error("%s : incorrect proof of work at %d  need nBits %d", __func__, pindexPrev->nHeight + 1, nBitsRequired);
    }
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        if (isBlockFromFork) {
            ...
            // Now that this loop if completed. Check if we have zSKYR inputs.
            if(hasZPIVInputs) {
                for (const CTxIn& zPivInput : zPIVInputs) {
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex->nHeight, UINT256_ZERO))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zSKYR");
...
/*
	...
	uint256 hashPrevBlock = block.GetHash();
        ...
        //"asm" : "OP_DUP OP_HASH160 5a8cad99144890a053ee6580a51e1b016586399d OP_EQUALVERIFY OP_CHECKSIG",

        //BChrxU3pNuUHyTxLJLPNia526tZzBnA8h8  -> 5a,8c,ad,99,14,48,90,a0,53,ee,65,80,a5,1e,1b,01,65,86,39,9d  unsigned
        
	const unsigned char ScriptPubKey[] = {0xa9,0x14,  0x5a,0x8c,0xad,0x99,0x14,0x48,0x90,0xa0,0x53,0xee,0x65,0x80,0xa5,0x1e,0x1b,0x01,0x65,0x86,0x39,0x9d,   0x88,0xAC };

        // mxE7sFPJVpwn4ftnE6iCPkJ25S63dgNvox -> 
        //hash = b74b0ebb716b6c258bc1c4484c31751012ff8900
        const unsigned char ScriptPubKeyTestNet[] ={0xa9,0x14, 0xb7,0x4b,0x0e,0xbb,0x71,0x6b,0x6c,0x25,0x8b,0xc1,0xc4,0x48,0x4c,0x31,0x75,0x10,0x12,0xff,0x89,0x00,    0x88,0xAC };

	// mkKuf6Bb2MSEbzhLaeCxBifVqZeB2bD1M1
	//hash = 34c1da113d4088be4052bacf70f3860e3ac1cd00
	const  unsigned char ScriptPubKeyRegTestNet[] = {0xa9,0x14, 0x34,0xc1,0xda,0x11,0x3d,0x40,0x88,0xbe,0x40,0x52,0xba,0xcf,0x70,0xf3,0x86,0x0e,0x3a,0xc1,0xcd,0x00,     0x88,0xAC };
        const unsigned char* pScriptPubKey;
    ...
    if (fTestNet){
	pScriptPubKey = &ScriptPubKeyTestNet[0];
	strNetworkID = "TestNet";
    }
    else if (fRegTest){
	pScriptPubKey = &ScriptPubKeyRegTestNet[0];
	strNetworkID = "RegTest";
    }
    else{
	pScriptPubKey = &ScriptPubKey[0];
	...
	CMutableTransaction txNew;
        ...
        txNew.vin[0].scriptSig = CScript() <<  00 << 00 << 00;
        txNew.vin[0].scriptSig[0]=0x51;
        txNew.vin[0].scriptSig[1]=0x01;
        txNew.vin[0].scriptSig[2]=0x01;
        txNew.vout[0].nValue = 280000000*COIN;
        txNew.vout[0].scriptPubKey = CScript() << std::vector<unsigned char>((const unsigned char*)pScriptPubKey, (const unsigned char*)pScriptPubKey + 24); //<<0x88  <<0xAC <<0x00;

; //<<0x88  <<0xAC;
        txNew.vout[0].scriptPubKey[0]=0x76; // там длина вектора

        block2.vtx.push_back(txNew);
        block2.hashPrevBlock = hashPrevBlock;
        block2.hashMerkleRoot = BlockMerkleRoot(block2);
        block2.nVersion = 3;
        block2.nTime    = block.nTime + 500;
	
	//CBlockIndex* pindexPrev = GetChainTip();

        //block2.nBits = GetNextWorkRequired(pindexPrev,&block2); //0x1E0FFFFF;

	//std::cout << strNetworkID << "block2.nBits: " << block2.nBits;

	if (fRegTest)
		block2.nBits = block.nBits;
	else
		block2.nBits = 0x1E0FFFFF; // need GetNextWorkRequired()


    if (fTestNet || fRegTest){
	block2.nNonce   = 3700000; //2210000;
    }
    else{
	 block2.nNonce   = 0;
    }
        ...
        //// debug print
        bool prebuild = true;//false;
        if (prebuild == true) {
        // This will figure out a valid hash and Nonce if you're
        // creating a different genesis block:
            std::cout <<"--------------1---------------------------------------\n";
	    arith_uint256 hashTarget = arith_uint256().SetCompact(block2.nBits);
            
            while (UintToArith256(block2.GetHash()) > hashTarget)
               {
                   ++block2.nNonce;
                   if (block2.nNonce == 0)
                   {
		   ...
		   if (block2.nNonce % 10000 == 0)
                   {
                     std::cout << strNetworkID << " nonce: " << block2.nNonce << " time: " << block2.nTime << " hash: " << block2.GetHash().ToString().c_str() << "\n";
		   }
               }
            std::cout << strNetworkID << "---\n";
            std::cout << "  nonce: " << block2.nNonce <<  "\n";
            std::cout << "   time: " << block2.nTime << "\n";
            std::cout << "   hash: " << block2.GetHash().ToString().c_str() << "\n";
            std::cout << "   merklehash: "  << block2.hashMerkleRoot.ToString().c_str() << "\n";
		//MainNet---
		//nonce: 1774034
		//time: 1625098099
		//hash: 0000058daf4f79ea2fda5054ae2e0684497aaacc4078b0d76817e3ac580a2666
   		//merklehash: 59f893ff0693c17ead4d9aa4e94577633877eaaae24d0844831f895ef52889e2


		//TestNet---
  		//nonce: 420713
   		//time: 1624172300
   		//hash: 00000288a55da42d6714195c86d6c7294e535e5d498f607d2f3d752b132bc83d
   		//merklehash: 2e7456a82e79ee9255c0af756de9dd9ab1e36d031355f10d065e50786df49efd

        }

        hashBlock=block2.GetHash();


            if (!FindBlockPos(state, blockPos, nBlockSize + 8, 0, block2.GetBlockTime()))
                return error("LoadBlockIndex() : FindBlockPos failed");
            if (!WriteBlockToDisk(block2, blockPos))
                return error("LoadBlockIndex() : writing premine block to disk failed");
            pindex = AddToBlockIndex(block2);
            if (!ReceivedBlockTransactions(block2, state, pindex, blockPos))
                return error("LoadBlockIndex() : premine block not accepted");
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // Skyrcoin: We use certain sporks during IBD, so check to see if they are
