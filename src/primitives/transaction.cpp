...
{
    // The wallet only allows publicSpend inputs in the same tx and not a combination between SKYR and zSKYR
    for(const CTxIn& txin : vin) {
...
bool CTransaction::HasP2CSOutputs() const	//<--- new
{
    for(const CTxOut& txout : vout) {
...
CAmount CTransaction::GetValueOut() const
{
    ...
    {
        // SKYR: previously MoneyRange() was called here. This has been replaced with negative check and boundary wrap check.
        if (it->nValue < 0)
