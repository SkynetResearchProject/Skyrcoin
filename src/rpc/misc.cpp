...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,           (numeric) the total SKYR balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"paytxfee\": x.xxxx,           (numeric) the transaction fee set in SKYR/kb\n"
            "  \"relayfee\": x.xxxx,           (numeric) minimum relay fee for non-free transactions in SKYR/kb\n"
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"SKYRaddress\"\n"
            "\nReturn information about the given SKYR address.\n"

            "\nArguments:\n"
            "1. \"SKYRaddress\"     (string, required) The SKYR address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"type\" : \"xxxx\",              (string) \"standard\"\n"
            "  \"address\" : \"SKYR address\",    (string) The SKYR address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: SKYR address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are SKYR addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) SKYR address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"SKYRaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"SKYRaddress\"  (string, required) The SKYR address to use for the signature.\n"
...
UniValue getstakingstatus(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"stakingbalance\": d                (numeric) SKYR value of the stakeable coins (minus reserve balance, if any)\n"
