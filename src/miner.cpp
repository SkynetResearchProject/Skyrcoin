...
{
    // double check that there are no double spent zSKYR spends in this block or tx
    if (tx.HasZerocoinSpendInputs()) {
        ...
        //This zSKYR serial has already been included in the block, do not add this tx.
        if (fDoubleSerial) {
            return false;
        }
    }
...

    {
        ...
        for (CTxMemPool::indexed_transaction_set::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                // zSKYR spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // zSKYR check to not include duplicated serials in the same block.
...
CBlockTemplate* CreateNewBlockWithKey(CReserveKey& reservekey, CWallet* pwallet)
{
    ...
    //if (nHeightNext==1)  // premine to
    //    //scriptPubKey = GetScriptForDestination(DecodeDestination("BChrxU3pNuUHyTxLJLPNia526tZzBnA8h8"));
    //    //scriptPubKey = GetScriptForDestination(DecodeDestination("mkKuf6Bb2MSEbzhLaeCxBifVqZeB2bD1M1")); // regtest
    //    scriptPubKey = GetScriptForDestination(DecodeDestination("mxE7sFPJVpwn4ftnE6iCPkJ25S63dgNvox")); // testnet
//
    //else
...
void BitcoinMiner(CWallet* pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            if (!consensus.NetworkUpgradeActive(pindexPrev->nHeight + 1, Consensus::UPGRADE_POS) || !fStakingActive) {
