...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:15888"));
...
/** Sanity checks
 *  Ensure that Skyrcoin is running in a usable environment with all
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. Skyrcoin is shutting down."));

    std::string strDataDir = GetDataDir().string();

    // Make sure only a single Skyrcoin process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. Skyrcoin is probably already running."), strDataDir));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //Skyrcoin specific: zerocoin and spork DB's
                ...
                // Skyrcoin: load previous sessions sporks if we have them.
                ...
                if (fReindexMoneySupply) {
                    LOCK(cs_main);
                    // Skip zSKYR if already reindexed
